...
//
// The recording is the point in more than one test: the operator's prompt is where the job is read, so
// "the service asked about the job that was actually signed" is a property worth holding.
type alwaysConfirm struct {
	// asked is the last request handed to the confirmation.
	asked JobConfirmation

	// calls counts how many times a human was asked.
	calls int
}

// confirm is the ConfirmJobFunc.
func (a *alwaysConfirm) confirm(req JobConfirmation) (bool, error) {
	a.calls++
	a.asked = req
	return true, nil
}

// testHost is the host every job request below is about.
const testHost = "01JTESTHOST0000000000000000"

// rebootRequest is a well-formed request for the one job these tests sign over and over.
const rebootRequest = `{"hostId":"` + testHost + `","intent":"host.reboot","params":{}}`

// newTestService builds a service and its HTTP handler.
func newTestService(t *testing.T, signer signing.Signer, confirm ConfirmJobFunc) http.Handler {
	t.Helper()
...
// signRequest posts a job to the signing endpoint with the headers a browser would send.
...
func TestNothingIsSignedWithoutAConfirmation(t *testing.T) {
	...
	handler := newTestService(t, signer, func(JobConfirmation) (bool, error) { return false, nil })

	response := httptest.NewRecorder()
	handler.ServeHTTP(response, signRequest(rebootRequest))
...
func TestAConfirmationThatCannotBeAskedIsNotAYes(t *testing.T) {
	signer := newStubSigner(t)
	handler := newTestService(t, signer, func(JobConfirmation) (bool, error) {
...
func TestAnOriginThatWasNotNamedIsRefused(t *testing.T) {
	...
	request := signRequest(rebootRequest)
...
func TestThePreflightAnswersThePrivateNetworkQuestion(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodOptions, SignJobPath, nil)
...
//
// Queueing would be worse than refusing: the operator would be reading one job and answering
...
func TestASecondRequestIsRefusedWhileSomebodyIsBeingAsked(t *testing.T) {
	...
	go func() {
		...
		handler.ServeHTTP(first, signRequest(rebootRequest))
		done <- first.Code
	}()
	<-asked

	second := httptest.NewRecorder()
	handler.ServeHTTP(second, signRequest(
		`{"hostId":"`+testHost+`","intent":"service.restart","params":{"unit":"nginx.service"}}`))
...
// anybody: a job signed by a key no host trusts is refused by every host, and nothing in the control
// plane can fix it. Putting the line where the operator already is saves the step that
...
func TestAServiceCannotBeBuiltWithoutTheThingsThatMakeItSafe(t *testing.T) {
	...
	for name, opts := range map[string]Options{
		"no signer":       {Origins: []string{testOrigin}, ConfirmJob: confirmJob},
		"no confirmation": {Signer: signer, Origins: []string{testOrigin}},
		"no origin":       {Signer: signer, ConfirmJob: confirmJob},
		"wildcard origin": {Signer: signer, ConfirmJob: confirmJob, Origins: []string{"*"}},
	} {
...
// TestASignedJobIsAssembledHereAndNotReceived is the property this package exists for.
//
...
func TestASignedJobIsAssembledHereAndNotReceived(t *testing.T) {
	...
	handler := newTestService(t, signer, func(req JobConfirmation) (bool, error) {
		asked = req
		return true, nil
	})

	response := httptest.NewRecorder()
	handler.ServeHTTP(response, signRequest(
...
// TestAJobRequestCannotChooseWhatBindsTheSignature is the refusing half of the same property.
//
...
func TestAJobRequestCannotChooseWhatBindsTheSignature(t *testing.T) {
	signer := newStubSigner(t)
	handler := newTestService(t, signer, func(JobConfirmation) (bool, error) { return true, nil })
...
//
// The address and the two paths are a contract between a Go program on an operator's machine and a
...
func TestTheBrowserAndTheSignerAgreeAboutWhereItListens(t *testing.T) {
	...
	for _, path := range []string{StatusPath, SignJobPath} {
